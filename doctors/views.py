...
class doctorViewSet(viewsets.GenericViewSet):
    ...
    def is_within_working_hours(self, user_time, working_hours, lunch_hours):
        if '휴무' in working_hours:
            return False
        #~로 구분된 시간을 24시간 형식으로 변환
        working_start, working_end = [int(self.convert_time_format(t)) for t in working_hours.split('~')]
        lunch_start, lunch_end = [int(self.convert_time_format(t)) for t in lunch_hours.split('~')]
    ...
    def searchDoctor(self, request):
        ...
        #datetime_str에 해당하는 의사를 찾아서 반환
            ...
            try:
                hour = int(self.convert_time_format(datetime_str.split(' ')[1]))
            except ValueError:
            ...
            available_doctors = [doctor for doctor in doctors if self.is_within_working_hours(user_time, doctor.time_business.split('/')[weekday], doctor.time_lunch)]
